...
import BasicTypes ( SourceText(..), Boxity(..), Activation(..)
...
mkTyConRepBinds stuff@(Stuff {..}) todo (TypeableTyCon {..})
  = do let prag = InlinePragma NoSourceText NoInline
                  Nothing AlwaysActive ConLike
       kind_rep_id <- (`setInlinePragma` prag)
